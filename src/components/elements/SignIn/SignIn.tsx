...
export const SignIn = () => {
  ...
  const signIn = () => {
    if (!formState.name.length)
      return enqueueSnackbar("заполните пустое поле", { variant: "warning" });
    if (!formState.password.length)
      return enqueueSnackbar("заполните пустое поле", { variant: "warning" });
    if (formState.password.length < 6)
      return enqueueSnackbar("пароль должен быть больше 6-ти символов", {
        variant: "warning",
      });
    ...
    const isNameInLS = usersInLS.find(
      (item: IUser) => item.name === formState.name
    );

    if (isNameInLS)
      return enqueueSnackbar("такое имя уже занято", { variant: "error" });
  ...
  return (
    <AnimatePresence>
      <div className={styles.inputBlock}>
        ...
        <button className={styles.enterBtn} onClick={signIn}>
          ENTER
        </button>
      </div>
    </AnimatePresence>
