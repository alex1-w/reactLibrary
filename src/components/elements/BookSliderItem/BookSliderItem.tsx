...
export const BookSliderItem: FC<{ book: IBook }> = ({ book }) => {
  ...
  const [isBookInCart, setIsBookInCart] = useState<boolean>(
    Boolean(user.cart?.find((item) => item.id === book.id))
  );
  const likeSvg = (
    <motion.svg
  ...
  const putInPlace = (book: IBook, place: "favorites" | "cart") => {
    ...
    if (!userInUsersLS)
      return enqueueSnackbar(`Вы не авторизованы`, { variant: "info" });
  ...
  return (
    <motion.div
      whileHover={{ boxShadow: "0px 7px 30px rgba(0, 0, 0, 0.30)" }}
    ...
    >
      ...
      <div className={styles.card__footer}>
        ...
        <div onClick={() => putInPlace(book, "favorites")}>{likeSvg}</div>
      </div>
    </motion.div>
  );
};
